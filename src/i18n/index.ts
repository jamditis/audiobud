...
// Overlapping calls (e.g. the startup sync racing a user's own language pick)
// each resume from their own `await loadLanguage`, so completion order does
// not otherwise match request order -- a slow early request could activate
// after, and overwrite, a faster later one. A monotonic ticket fixes that: a
// request only activates i18n's active language if no newer request has
// started since it began loading.
let latestChangeRequestId = 0;
...
export const changeAppLanguage = async (
  ...
  const requestId = ++latestChangeRequestId;
  await loadLanguage(supported);

  // A newer request started while this one was loading; let it win instead.
  if (requestId !== latestChangeRequestId) return true;
