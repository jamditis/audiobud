...
async changeFormatRawOutputSetting(enabled: boolean) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_format_raw_output_setting", { enabled }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async isUpdateChannelAvailable() : Promise<boolean> {
    return await TAURI_INVOKE("is_update_channel_available");
},
 ...
 */
export type TranscriptDeliveredEvent = { app: string | null; title: string | null }
