...
- [ ] **#1409 — post-processing runs on empty transcription.** `src-tauri/src/actions.rs:363-377`
  ...
  with no `final_text.trim().is_empty()` guard (the empty-text guard at `:602` is *after* the LLM
...
- [ ] **#1423 — wrong tray icon on Windows dark/custom mode (interacts with our icon rebrand).**
  ...
  Windows taskbar follows the *system* mode, which can differ ("custom") → our new dark-variant icon
...
- [ ] **#537 / #554 — SIGILL on CPUs without FMA3/AVX2.** The FMA guard (`transcription.rs:789`)
  only protects GPU *enumeration*, not the whisper-vulkan inference or ONNX/Parakeet load. Crash
